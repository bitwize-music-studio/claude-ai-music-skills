...
- **A persistently bad override value warned on every read instead of once** ([#556](https://github.com/bitwize-music-studio/claude-ai-music-skills/issues/556)) — `coerce_yaml_bool`/`coerce_yaml_float` (`tools/shared/config.py`) logged the same warning text on every call, so one bad genre-scoped setting produced the identical line once per stem per track across an album. Each distinct `(setting, bad value)` pair now warns once per process; a *different* bad value for the same setting still warns, and the warning text itself is unchanged.
- **Genre-scoped mix overrides now apply without passing `genre` explicitly, and `polish_album`'s analyze and polish stages can no longer disagree about which genre they're using** ([#556](https://github.com/bitwize-music-studio/claude-ai-music-skills/issues/556)) — `polish_album`'s stage-1 call into `analyze_mix_issues` dropped the `genre` argument entirely, so an explicit `polish_album(album, genre="hip-hop")` resolved stage 1's analyzer settings (e.g. `click_peak_ratio`) against the defaults while stage 2's polish resolved them against `hip-hop` — the two stages of one run disagreeing about the same setting. That call now forwards `genre`. Separately, `polish_audio`, `polish_album` and `analyze_mix_issues` now default an omitted `genre` from the album's own genre, read from the state cache — the same authoritative source `_resolve_audio_dir` already reads to build the album's path, rather than re-deriving it by splitting the resolved directory back apart. An explicit `genre` argument always wins; a derivation failure (album missing from state, no genre recorded) falls back to today's no-genre behavior rather than erroring, and a derived genre goes through the same unknown-genre validation as an explicit one. `polish_album` resolves genre once and forwards the identical value to both stages, so a plain `polish_audio(album_slug)` or `polish_album(album_slug)` call now applies that album's genre-scoped overrides automatically.
