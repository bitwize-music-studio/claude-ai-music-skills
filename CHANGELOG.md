...
### Changed
- **`reference/suno/v5-best-practices.md` moved to `reference/suno/version-history/`** ([#562](https://github.com/bitwize-music-studio/claude-ai-music-skills/issues/562)) — archived verbatim with a retired banner; every link, the `get_reference` docstring, `tests/plugin/test_references.py` and `skills/test/test-definitions.md` now point at `best-practices.md`. `skills/pre-generation-check/SKILL.md` also stops describing the Explicit gate as a warning; the MCP gate has blocked on an unset flag since #370.
