...
- **Genre-scoped mix overrides now apply without passing `genre` explicitly, and `polish_album`'s analyze and polish stages can no longer disagree about which genre they're using** ([#556](https://github.com/bitwize-music-studio/claude-ai-music-skills/issues/556)) — `polish_album`'s stage-1 call into `analyze_mix_issues` dropped the `genre` argument entirely, so an explicit `polish_album(album, genre="hip-hop")` resolved stage 1's analyzer settings (e.g. `click_peak_ratio`) against the defaults while stage 2's polish resolved them against `hip-hop` — the two stages of one run disagreeing about the same setting. That call now forwards `genre`. Separately, `polish_audio`, `polish_album`, `analyze_mix_issues` and `polish_and_master_album` now default an omitted `genre` from the album's own genre, read from the state cache — the same authoritative source `_resolve_audio_dir` already reads to build the album's path, rather than re-deriving it by splitting the resolved directory back apart. An explicit `genre` argument always wins; a derivation failure (album missing from state, no genre recorded) falls back to today's no-genre behavior rather than erroring. `polish_album` resolves genre once and forwards the identical value to both stages, and `polish_and_master_album` does the same across both phases, so a plain `polish_audio(album_slug)`, `polish_album(album_slug)` or `polish_and_master_album(album_slug)` call now applies that album's genre-scoped overrides automatically.
  - **A DERIVED genre unrecognized by a preset set is a soft fallback, not a hard error — unlike an explicit one.** The first cut of this fix validated a derived genre exactly like an explicit one, which reintroduced a real regression: an album whose own recorded genre (e.g. a niche "dark-cabaret") simply isn't among a preset file's genre list started hard-failing `polish_audio`/`polish_album`/`polish_and_master_album`'s polish stage on every call, with no escape hatch — passing `genre=""` just re-derives the same value. The hard "Unknown genre" error now stays reserved for a genre the caller actually typed; a DERIVED one is never hard-errored. What happens next differs by preset set, because they tolerate an unrecognized genre differently: `master_album`'s pipeline validates strictly with no fallback of its own, so `polish_and_master_album`'s master phase (and `polish_album`'s stage-3 QC guard, which hit the identical `qc_track` `ValueError` — see below) still warn and drop the genre for that step. The MIX side does the opposite: `_get_stem_settings`/`_get_full_mix_settings`/`_resolve_analyzer_peak_ratio` already resolve a mix-unknown genre gracefully (shipped per-stem defaults, plus the mastering-preset click-threshold overlay, which reads the SEPARATE mastering preset set and can still recognize the genre there), so `polish_audio`/`polish_album`/`polish_and_master_album`'s polish phase now just log that the genre has no mix-preset section and KEEP it, unchanged, rather than dropping it. Dropping it (the fix's first cut) was itself a live bug: `analyze_mix_issues` never validated genre at all, so a `polish_album` run's stage 1 (analyzer, unblanked) and stage 2 (polish, blanked) independently re-derived the same state genre and reached *different* effective settings for the same run — the exact stage-disagreement D3 item 1 exists to prevent, now reproduced by the fix meant to prevent it. Forwarding a mix-unknown DERIVED genre unchanged from `polish_album`/`polish_and_master_album` down into `polish_audio` would trip *its* hard error, since `polish_audio` cannot tell a value it derived-and-was-handed apart from one a caller typed — both orchestrators now forward `""` instead in that one case, letting `polish_audio` re-derive (identical state, identical result) and correctly resolve the value as derived rather than explicit. `polish_album`'s stage-3 QC guard against `qc_track`'s `ValueError` (a genre absent from the mastering presets that reaches `qc_track` crashed `run_in_executor` outright) now checks `master_tracks.GENRE_PRESETS` — the import-time snapshot `qc_track`'s own `_resolve_click_thresholds` actually reads — rather than a fresh `load_genre_presets()` call, which could disagree with the snapshot after a mid-session override edit and let the guard pass while the crash still fired underneath. All the drop-and-warn cases (master phase, QC guard) share one dedup key per (album, genre, preset set) via the same warn-once mechanism `coerce_yaml_bool`/`coerce_yaml_float` use (above), so one run that discovers the same fact from more than one call site logs it once; the mix-side keep-and-inform case is deduped the same way, separately.
